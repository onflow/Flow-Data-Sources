# Source: https://github.com/IncrementFi/Liquid-Staking/blob/main/cadence/contracts/standard/mainnet/FlowClusterQC.cdc
...
access(all) contract FlowClusterQC {
    ...
    access(all) struct Cluster {
        ...
        view init(index: UInt16, nodeWeights: {String: UInt64}) {
            self.index = index
        ...
        access(all) view fun voteThreshold(): UInt64 {
            ...
            let floorOneThird = self.totalWeight / 3 // integer division, includes floor

            var res = floorOneThird * 2

            let divRemainder = self.totalWeight % 3
