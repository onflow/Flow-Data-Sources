# Source: https://github.com/OurThinkTank/Flow/blob/main/contracts/InfiniteObjects_NFT.cdc

```

import NonFungibleToken from 0x1d7e57aa55817448

pub contract InfiniteObjects_NFT: NonFungibleToken {

    // InfiniteObjects_NFT Events
    //
    // Emitted when the InfiniteObjects_NFT contract is created
    ...
    // totalSupply
    // The total number of InfiniteObjects_NFT that have been minted
    //
    ...
    // A Series is special resource type that contains functions to mint InfiniteObjects_NFT NFTs, 
	...
	pub resource Series {
        ...
        init(
            ...
            InfiniteObjects_NFT.seriesData[seriesId] = SeriesData(
                    seriesId: seriesId,
                    metadata: metadata
            )      
        }
        ...
        pub fun addNftSet(
            ...
            InfiniteObjects_NFT.setData[setId] = newNFTSet
        ...
        pub fun updateSeriesMetadata(metadata: {String: String}) {
            ...
            InfiniteObjects_NFT.seriesData[self.seriesId] = newSeriesMetadata
	    ...
	    pub fun mintInfiniteObjects_NFT(
            ...
            pre {
                ...
                self.numberEditionsMintedPerSet[setId]! <= InfiniteObjects_NFT.getSetMaxEditions(setId: setId)!:
			...
			recipient.deposit(token: <-create InfiniteObjects_NFT.NFT(
                tokenId: tokenId,
                setId: setId,
    ...
    // A resource that represents the InfiniteObjects_NFT NFT
    //
    pub resource NFT: NonFungibleToken.INFT {
        ...
        init(
          ...
          editionNum: UInt32) {
            ...
            let seriesId = InfiniteObjects_NFT.getSetSeriesId(setId: setId)!
    ...
    pub resource Admin {
        ...
        pub fun addSeries(seriesId: UInt32, metadata: {String: String}) {
            pre {
                InfiniteObjects_NFT.series[seriesId] == nil:
            ...
            InfiniteObjects_NFT.series[seriesId] <-! newSeries
        }
        ...
        pub fun borrowSeries(seriesId: UInt32): &Series  {
            ...
            return &InfiniteObjects_NFT.series[seriesId] as &Series
        }
    ...
    // to allow others to deposit InfiniteObjects_NFT into their Collection. It also allows for reading
    // the details of InfiniteObjects_NFT in the Collection.
    pub resource interface InfiniteObjects_NFTCollectionPublic {
        pub fun deposit(token: @NonFungibleToken.NFT)
        pub fun batchDeposit(tokens: @NonFungibleToken.Collection)
        pub fun getIDs(): [UInt64]
        pub fun borrowNFT(id: UInt64): &NonFungibleToken.NFT
        pub fun borrowInfiniteObjects_NFT(id: UInt64): &InfiniteObjects_NFT.NFT? {
            // If the result isn't nil, the id of the returned reference
            // should be the same as the argument to the function
            post {
                (result == nil) || (result?.id == id):
                    "Cannot borrow InfiniteObjects_NFT reference: The ID of the returned reference is incorrect"
            }
        }
    }

    // Collection
    // A collection of InfiniteObjects_NFT NFTs owned by an account
    //
    pub resource Collection: InfiniteObjects_NFTCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic {
        ...
        //
        pub fun deposit(token: @NonFungibleToken.NFT) {
            let token <- token as! @InfiniteObjects_NFT.NFT
        ...
        // borrowInfiniteObjects_NFT
        // Gets a reference to an NFT in the collection as a InfiniteObjects_NFT,
        // exposing all of its fields.
        // This is safe as there are no functions that can be called on the InfiniteObjects_NFT.
        //
        pub fun borrowInfiniteObjects_NFT(id: UInt64): &InfiniteObjects_NFT.NFT? {
            if self.ownedNFTs[id] != nil {
                let ref = &self.ownedNFTs[id] as auth &NonFungibleToken.NFT
                return ref as! &InfiniteObjects_NFT.NFT
            } else {
                return nil
            }
        }
    ...
    // fetch
    // Get a reference to a InfiniteObjects_NFT from an account's Collection, if available.
    // If an account does not have a InfiniteObjects_NFT.Collection, panic.
    ...
    //
    pub fun fetch(_ from: Address, id: UInt64): &InfiniteObjects_NFT.NFT? {
        let collection = getAccount(from)
            .getCapability(InfiniteObjects_NFT.CollectionPublicPath)
            .borrow<&InfiniteObjects_NFT.Collection{InfiniteObjects_NFT.InfiniteObjects_NFTCollectionPublic}>()
            ?? panic("Couldn't get collection")
        // We trust InfiniteObjects_NFT.Collection.borrowInfiniteObjects_NFT to get the correct id
        // (it checks it before returning it).
        return collection.borrowInfiniteObjects_NFT(id: id)
    }

    // getAllSeries returns all the sets
    //
    // Returns: An array of all the series that have been created
    pub fun getAllSeries(): [InfiniteObjects_NFT.SeriesData] {
        return InfiniteObjects_NFT.seriesData.values
    }

    // getAllSets returns all the sets
    //
    // Returns: An array of all the sets that have been created
    pub fun getAllSets(): [InfiniteObjects_NFT.NFTSetData] {
        return InfiniteObjects_NFT.setData.values
    }
    ...
    pub fun getSeriesMetadata(seriesId: UInt32): {String: String}? {
        return InfiniteObjects_NFT.seriesData[seriesId]?.getMetadata()
    }
    ...
    pub fun getSetMaxEditions(setId: UInt32): UInt32? {
        return InfiniteObjects_NFT.setData[setId]?.maxEditions
    }
    ...
    pub fun getSetMetadata(setId: UInt32): {String: String}? {
        return InfiniteObjects_NFT.setData[setId]?.getMetadata()
    }
    ...
    pub fun getSetSeriesId(setId: UInt32): UInt32? {
        return InfiniteObjects_NFT.setData[setId]?.seriesId
    }
    ...
    pub fun getIpfsMetadataHashByNftEdition(setId: UInt32, editionNum: UInt32): String? {
        ...
        if let set = InfiniteObjects_NFT.setData[setId] {
	...
	init() {
        // Set named paths
        self.CollectionStoragePath = /storage/InfiniteObjects_NFTCollection
        self.CollectionPublicPath = /public/InfiniteObjects_NFTCollection
        self.AdminStoragePath = /storage/InfiniteObjects_NFTAdmin
        self.AdminPrivatePath = /private/InfiniteObjects_NFTAdminUpgrade
        ...
        self.account.link<&InfiniteObjects_NFT.Admin>(
