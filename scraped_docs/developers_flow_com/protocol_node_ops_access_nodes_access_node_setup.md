# Source: https://developers.flow.com/protocol/node-ops/access-nodes/access-node-setup
...
This guide is for running a permissonless Access node on Flow. If you are planning to run a different type of staked node then see [node bootstrap](/protocol/node-ops/node-operation/node-bootstrap).
...
You can read more about epochs [here](/protocol/staking/schedule).
...
To run an access node you will need to provision a machine or virtual machine to run your node software. Please follow the [node-provisioning](/protocol/node-ops/node-operation/node-provisioning) guide for it.
...
[Here](/protocol/flow-port/staking-guide) is a guide on how to use Flow port if you are not familiar with it.
If you are staking via a custody provider or would like to directly submit a staking transaction instead follow this [guide](/protocol/staking#how-do-i-stake).
...
Alternatively, if you can monitor events, look for [the epoch setup event](/protocol/staking/epoch-scripts-events#flowepochepochsetup) that gets emitted by the epoch contract. That event is emitted at the end of epoch N's staking auction and contains a list of node IDs that are confirmed for the next epoch.
...
First you'll need to provision a machine or virtual machine to run your node software. Please see follow the [node-provisioning](/protocol/node-ops/node-operation/node-provisioning) guide for it.
...
For a more mature setup, it is recommended that you run the container using systemd as described [here](/protocol/node-ops/node-operation/node-setup#systemd)
...
The node publishes several Prometheus metrics. See [Monitoring Node Health](/protocol/node-ops/node-operation/monitoring-nodes) to setup node monitoring.
...
Node Operations](/protocol/node-ops)[Next
