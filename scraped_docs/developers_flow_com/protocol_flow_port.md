# Source: https://developers.flow.com/protocol/flow-port
...
For a detailed walkthrough on how to use Flow Port for staking and delegating, please read the [Flow Port staking walkthrough](/protocol/flow-port/staking-guide)
...
1. You need to have FLOW in order to stake. Please see the [FLOW Token](/build/cadence/core-contracts/flow-token) reference for information on how to become a FLOW holder.
...
If you are not using a custody provider, there is more responsibility that you have to accept, because you have complete control of your tokens. You need to ensure that you are well informed about the staking process and potentially node operation process because you will have to manage those on your own. Please read the [staking documentation](/protocol/staking) before continuing with this guide.
...
4. Run the [bootstrapping instructions](/protocol/node-ops/node-operation/node-bootstrap) and provide the remaining technical details needed to stake a node.
...
* Now that you have executed a stake transaction in either Flow Port or your custody provider’s portal, that transaction will sit in a pending status until it is processed, which will be at the next [Epoch](/protocol/staking#epochs) Date (which is currently weekly).
* During the next [Epoch](/protocol/staking#epochs), the transaction will be processed. If successful, the provided FLOW will be staked and the associated Node would be either **a)** included in the network protocol if it is a new node or **b)** continue to operate as is in the network protocol.
...
* Withdraw/re-stake your earned rewards. If you decide to withdraw your rewards, this action will happen instantly. If you decide to re-stake your rewards, the request will again sit in a pending status and will be processed at the next [Epoch](/protocol/staking#epochs).
...
[Edit this page](https://github.com/onflow/docs/tree/main/docs/protocol/flow-port/index.md)

Last updated on **Sep 16, 2025** by **Pandit Dhamdhere**

[Previous

Governance](/protocol/governance)[Next
