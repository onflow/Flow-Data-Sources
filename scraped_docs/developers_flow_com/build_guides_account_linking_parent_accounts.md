...
withdraw\_nft\_from\_child.cdc `_51import "NonFungibleToken"_51import "FlowToken"_51import "HybridCustody"_51_51transaction(_51 childAddress: Address, // Address of the child account_51 storagePath: StoragePath, // Path to the Collection in the child account_51 collectionType: Type, // Type of the requested Collection from which to withdraw_51 withdrawID: UInt64 // ID of the NFT to withdraw_51 ) {_51_51 let providerRef: &{NonFungibleToken.Provider}_51_51 prepare(signer: auth(BorrowValue) &Account) {_51 // Get a reference to the signer's HybridCustody.Manager from storage_51 let managerRef = signer.storage.borrow<auth(HybridCustody.Manage) &HybridCustody.Manager>(_51 from: HybridCustody.ManagerStoragePath_51 ) ?? panic("Could not borrow reference to HybridCustody.Manager in signer's account at expected path!")_51_51 // Borrow a reference to the signer's specified child account_51 let account = managerRef_51 .borrowAccount(addr: childAddress)_51 ?? panic("Signer does not have access to specified child account")_51_51 // Get the Capability Controller ID for the requested collection type_51 let controllerID = account.getControllerIDForType(_51 type: collectionType,_51 forPath: storagePath_51 ) ?? panic("Could not find Capability controller ID for collection type ".concat(type.identifier)_51 .concat(" at path ").concat(storagePath.toString()))_51_51 // Get a reference to the child NFT Provider and assign to the transaction scope variable_51 let cap = account.getCapability(_51 controllerID: controllerID,_51 type: Type<auth(NonFungibleToken.Withdraw) &{NonFungibleToken.Provider}>()_51 ) ?? panic("Cannot access NonFungibleToken.Provider from this child account")_51_51 // We'll need to cast the Capability - this is possible thanks to CapabilityFactory, though we'll rely on the relevant_51 // Factory having been configured for this Type or it won't be castable_51 self.providerRef = cap as! Capability<auth(NonFungibleToken.Withdraw) &{NonFungibleToken.Provider}>_51 }_51_51 execute {_51 // Withdraw the NFT from the Collection_51 let nft <- self.providerRef.withdraw(withdrawID: withdrawID)_51 // Do stuff with the NFT_51 // NOTE: Without storing or burning the NFT before scope closure, this transaction will fail. You'll want to_51 // fill in the rest of the transaction with the necessary logic to handle the NFT_51 // ..._51 }_51}`
...
[Edit this page](https://github.com/onflow/docs/tree/main/docs/build/guides/account-linking/parent-accounts.md)Last updated on **Jan 10, 2025** by **Brian Doyle**[PreviousBuilding Walletless Applications Using Child Accounts](/build/guides/account-linking/child-accounts)[NextAccount Linking With NBA Top Shot](/build/guides/account-linking-with-dapper)
###### Rate this page
