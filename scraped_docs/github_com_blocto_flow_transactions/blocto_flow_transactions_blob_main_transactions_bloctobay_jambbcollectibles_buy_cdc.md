# Source: https://github.com/blocto/flow-transactions/blob/main/transactions/BloctoBay/jambbCollectibles/buy.cdc
...
transaction(listingResourceID: UInt64, storefrontAddress: Address, buyPrice: UFix64) {
    ...
    prepare(signer: AuthAccount) {
        ...
        if signer.borrow<&Collectibles.Collection>(from: Collectibles.CollectionStoragePath) == nil {
            signer.save(<- Collectibles.createEmptyCollection(), to: Collectibles.CollectionStoragePath)
            signer.link<&{NonFungibleToken.CollectionPublic, Collectibles.CollectionPublic}>(
        ...
        self.nftCollection = signer.borrow<&{NonFungibleToken.Receiver}>(from: Collectibles.CollectionStoragePath)
