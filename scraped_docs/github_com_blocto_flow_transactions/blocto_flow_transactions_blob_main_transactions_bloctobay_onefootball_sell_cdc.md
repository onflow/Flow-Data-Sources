# Source: https://github.com/blocto/flow-transactions/blob/main/transactions/BloctoBay/oneFootball/sell.cdc
...
import OneFootballCollectible from 0xONE_FOOTBALL_COLLECTIBLE_ADDRESS
...
transaction(saleItemID: UInt64, saleItemPrice: UFix64) {
    ...
    prepare(signer: AuthAccount) {
        ...
        let nftCollectionProviderPrivatePath = /private/OneFootballCollectibleCollectionProviderForNFTStorefront
        if !signer.getCapability<&OneFootballCollectible.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(nftCollectionProviderPrivatePath)!.check() {
            signer.link<&OneFootballCollectible.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(nftCollectionProviderPrivatePath, target: OneFootballCollectible.CollectionStoragePath)
        }
    ...
    execute {
        // Remove old listing
        if let listingID = Marketplace.getListingID(nftType: Type<@OneFootballCollectible.NFT>(), nftID: saleItemID) {
        ...
        let requirements = Marketplace.getSaleCutRequirements(nftType: Type<@OneFootballCollectible.NFT>())
        ...
        let id = self.storefront.createListing(
            ...
            nftType: Type<@OneFootballCollectible.NFT>(),
            nftID: saleItemID,
