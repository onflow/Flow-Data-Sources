# Source: https://github.com/blocto/flow-transactions/blob/main/transactions/BloctoBay/youtube/sell.cdc
...
import Art_NFT from 0xYOUTUBE_ART_NFT_ADDRESS
...
transaction(saleItemID: UInt64, saleItemPrice: UFix64) {
    ...
    prepare(signer: AuthAccount) {
        ...
        let nftCollectionProviderPrivatePath = /private/Art_NFTCollectionProviderForNFTStorefront
        if !signer.getCapability<&Art_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(nftCollectionProviderPrivatePath)!.check() {
            signer.link<&Art_NFT.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(nftCollectionProviderPrivatePath, target: Art_NFT.CollectionStoragePath)
        }
    ...
    execute {
        // Remove old listing
        if let listingID = Marketplace.getListingID(nftType: Type<@Art_NFT.NFT>(), nftID: saleItemID) {
        ...
        let requirements = Marketplace.getSaleCutRequirements(nftType: Type<@Art_NFT.NFT>())
