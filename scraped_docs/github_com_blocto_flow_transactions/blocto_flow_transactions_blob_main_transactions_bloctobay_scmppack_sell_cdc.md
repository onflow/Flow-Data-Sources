# Source: https://github.com/blocto/flow-transactions/blob/main/transactions/BloctoBay/scmpPack/sell.cdc
...
import ARTIFACTPack from 0xSCMP_ARTIFACTPACK_ADDRESS
...
transaction(saleItemID: UInt64, saleItemPrice: UFix64) {
    ...
    prepare(signer: AuthAccount) {
        ...
        let nftCollectionProviderPrivatePath = /private/ARTIFACTPackCollectionProviderForNFTStorefront
        if !signer.getCapability<&ARTIFACTPack.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(nftCollectionProviderPrivatePath)!.check() {
            signer.link<&ARTIFACTPack.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(nftCollectionProviderPrivatePath, target: ARTIFACTPack.collectionStoragePath)
        }
    ...
    execute {
        // Remove old listing
        if let listingID = Marketplace.getListingID(nftType: Type<@ARTIFACTPack.NFT>(), nftID: saleItemID) {
        ...
        let requirements = Marketplace.getSaleCutRequirements(nftType: Type<@ARTIFACTPack.NFT>())
