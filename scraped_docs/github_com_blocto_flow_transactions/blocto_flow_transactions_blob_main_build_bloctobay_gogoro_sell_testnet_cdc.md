# Source: https://github.com/blocto/flow-transactions/blob/main/build/BloctoBay/gogoro/sell.testnet.cdc
...
transaction(saleItemID: UInt64, saleItemPrice: UFix64) {
    ...
    prepare(signer: AuthAccount) {
        ...
        let nftCollectionProviderPrivatePath = /private/GogoroCollectibleCollectionProviderForNFTStorefront
        if !signer.getCapability<&GogoroCollectible.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(nftCollectionProviderPrivatePath)!.check() {
            signer.link<&GogoroCollectible.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(nftCollectionProviderPrivatePath, target: GogoroCollectible.CollectionStoragePath)
        }
    ...
    execute {
        // Remove old listing
        if let listingID = Marketplace.getListingID(nftType: Type<@GogoroCollectible.NFT>(), nftID: saleItemID) {
        ...
        let requirements = Marketplace.getSaleCutRequirements(nftType: Type<@GogoroCollectible.NFT>())
        ...
        let id = self.storefront.createListing(
            ...
            nftType: Type<@GogoroCollectible.NFT>(),
            nftID: saleItemID,
