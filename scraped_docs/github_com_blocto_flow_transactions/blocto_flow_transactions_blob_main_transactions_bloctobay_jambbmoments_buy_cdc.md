# Source: https://github.com/blocto/flow-transactions/blob/main/transactions/BloctoBay/jambbMoments/buy.cdc
...
transaction(listingResourceID: UInt64, storefrontAddress: Address, buyPrice: UFix64) {
    ...
    prepare(signer: AuthAccount) {
        ...
        if signer.borrow<&Moments.Collection>(from: Moments.CollectionStoragePath) == nil {
            signer.save(<- Moments.createEmptyCollection(), to: Moments.CollectionStoragePath)
            signer.link<&{NonFungibleToken.CollectionPublic, Moments.CollectionPublic}>(
        ...
        self.nftCollection = signer.borrow<&{NonFungibleToken.Receiver}>(from: Moments.CollectionStoragePath)
