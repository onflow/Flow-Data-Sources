# Source: https://github.com/blocto/flow-transactions/blob/main/transactions/BloctoBay/jambbCollectibles/sell.cdc
...
import Collectibles from 0xJAMBB_COLLECTIBLES_ADDRESS
...
transaction(saleItemID: UInt64, saleItemPrice: UFix64) {
    ...
    prepare(signer: AuthAccount) {
        ...
        let nftCollectionProviderPrivatePath = /private/CollectiblesCollectionProviderForNFTStorefront
        if !signer.getCapability<&Collectibles.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(nftCollectionProviderPrivatePath)!.check() {
            signer.link<&Collectibles.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>(nftCollectionProviderPrivatePath, target: Collectibles.CollectionStoragePath)
        }
    ...
    execute {
        // Remove old listing
        if let listingID = Marketplace.getListingID(nftType: Type<@Collectibles.NFT>(), nftID: saleItemID) {
        ...
        let requirements = Marketplace.getSaleCutRequirements(nftType: Type<@Collectibles.NFT>())
